                ...
                <div class="header-body">
                    <div class="row align-items-center py-4">
                        <div class="col-lg-6 col-7">
                            <h6 class="h2 text-white d-inline-block mb-0">Hydration Tracker</h6>
                            <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Hydration Tracker</li>
                                </ol>
                            </nav>
                        </div>
                ...
                <div class="col">
                    <div class="card">
                        ...
                        <div class="table-responsive">
                                    ...
                                    <div class="row">
                                        <div class="col-lg">
                                            <div class="form-group">
                                                <label class="form-control-label" for="glasses">Glasses of Water</label>
                                                <input v-model="entry.glasses" type="range" class="form-control-range" min="0" max="8" name="glasses">
                                                <div class="text-center w-100 mt-2">{{ entry.glasses }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <HydrationLogEntries v-on:updateEntry="editEntry" v-on:updateLogs="getEntries()" :entries="logEntries" />
                        </div>
                        <!-- Card footer -->
...
<script>
import HydrationLogEntries from '../components/HydrationLogEntries.vue';
...
export default {
    ...
    methods: {
        ...
        submitEntry() {
            if (this.entry.glasses == "" ||
                this.entry.description == "") {
                this.error.push("please fill in all the fields");
                return;
            } else {
                this.error = [];
                axios.post('api/hydrationlog/store', {
                    hydrationlog: this.entry,
                })
                .then(response => {
                    if (response.status == 201) {
                        this.entry.glasses = 4;
                        this.entry.description = "";
                        this.getEntries();
                    }
                })
        ...
        editEntry(id) {
            ...
            this.logEntries.forEach(entry => {
                if (entry.id == id) {
                    this.entry.glasses = entry.glasses;
        ...
        updateEntry() {
            axios.put('api/hydrationlog/' + this.editMode, {
                hydrationlog: this.entry,
            })
            .then(response => {
